...
public class NotificationListener extends NotificationListenerService {
  ...
  private static HashMap<String, ArrayList<NotificationCallback>>
      _currentNotifications = new HashMap<>();
  ...
  public void onListenerConnected() {
    ...
    mMonitorHandler.sendMessage(
        mMonitorHandler.obtainMessage(EVENT_UPDATE_CURRENT_NOS));
  }

  @Override
  public void onNotificationPosted(StatusBarNotification sbn) {
    // Some apps do not track total notifications in their
    // StatusBarNotification; given this is an onNotificationPosted, ensure we
    // have a minimum count to display the badge, otherwise it will be removed
  ...
  private void processCallback(String packageName, int count) {
    ...
    ArrayList<NotificationCallback> callbacks =
        _currentNotifications.get(packageName);
  ...
  public static void setNotificationCallback(String packageName,
  ...
  class NotificationListenerReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
      if (intent.getStringExtra(UPDATE_NOTIFICATIONS_COMMAND)
              .equals(UPDATE_NOTIFICATIONS_UPDATE)) {
