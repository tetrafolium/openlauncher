...
public class SearchBar extends FrameLayout {
  ...
  private void init() {
    ...
    // These have to match the Preferences Array, but without item 0 as that is
    // a custom option which can be changed:
    ...
    _switchButton.setOnClickListener(new OnClickListener() {
      ...
      public void onClick(View view) {
        Setup.appSettings().setSearchUseGrid(
            !Setup.appSettings().getSearchUseGrid());
        updateSwitchIcon();
    ...
    LayoutParams switchButtonParams =
        new LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT,
                         ViewGroup.LayoutParams.WRAP_CONTENT);
    ...
    Setup.appLoader().addUpdateListener(new AppUpdateListener() {
      ...
      public boolean onAppUpdated(List<App> apps) {
        ...
        for (int i = 0; i < apps.size(); i++) {
          ...
          items.add(
              new IconLabelItem(app.getIcon(), app.getLabel())
                  .withIconSize(50)
                  .withTextColor(Color.WHITE)
                  .withIsAppLauncher(true)
                  .withIconPadding(8)
                  .withOnClickAnimate(false)
                  .withTextGravity(Setup.appSettings().getSearchUseGrid()
                                       ? Gravity.CENTER
                                       : Gravity.CENTER_VERTICAL)
                  .withIconGravity(Setup.appSettings().getSearchUseGrid()
                                       ? Gravity.TOP
                                       : Gravity.START)
                  ...
                  .withOnLongClickListener(DragHandler.getLongClick(
                      Item.newAppItem(app), DragAction.Action.SEARCH, null)));
        }
        _adapter.set(items);
    ...
    _adapter.getItemFilter().withFilterPredicate(
        new IItemAdapter.Predicate<IconLabelItem>() {
          @Override
    ...
    final LayoutParams recyclerParams =
        new LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
                         ViewGroup.LayoutParams.WRAP_CONTENT);
    ...
    _searchInput.getViewTreeObserver().addOnGlobalLayoutListener(
        new ViewTreeObserver.OnGlobalLayoutListener() {
          @Override
          public void onGlobalLayout() {
            _searchInput.getViewTreeObserver().removeOnGlobalLayoutListener(
                this);
  ...
  private void collapseInternal() {
    ...
    Tool.goneViews(ANIM_TIME, _searchCardContainer, _searchRecycler,
                   _switchButton);
  ...
  private void expandInternal() {
    ...
    Tool.visibleViews(ANIM_TIME, _searchCardContainer, _searchRecycler,
                      _switchButton);
    Tool.goneViews(ANIM_TIME, _searchClock);
  }

  private void updateSwitchIcon() {
    _switchButton.setImageResource(Setup.appSettings().getSearchUseGrid()
                                       ? R.drawable.ic_view_grid_white
                                       : R.drawable.ic_view_list_white);
  }

  private void updateRecyclerViewLayoutManager() {
    int gridSize = Setup.appSettings().getSearchUseGrid()
                       ? Setup.appSettings().getDrawerColumnCount()
                       : 1;
    if (gridSize == 1) {
      _searchRecycler.setLayoutManager(new LinearLayoutManager(
          getContext(), LinearLayoutManager.VERTICAL, false));
      updateList(Gravity.START, Gravity.CENTER_VERTICAL);
    } else {
      _searchRecycler.setLayoutManager(new GridLayoutManager(
          getContext(), gridSize, GridLayoutManager.VERTICAL, false));
  ...
  public AppCompatImageView getSearchButton() { return _searchButton; }
  ...
  public void updateClock() {
    ...
    _clockFormatter =
        getSearchBarClockFormat(Setup.appSettings().getDesktopDateMode());

    String text = now.format(_clockFormatter);
    String[] lines = text.split("\n");
    Spannable span = new SpannableString(text);
    span.setSpan(new RelativeSizeSpan(_searchClockSubTextFactor),
                 lines[0].length() + 1,
