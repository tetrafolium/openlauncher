...
public final class PagerIndicator
	extends View implements ViewPager.OnPageChangeListener {
	...
	case Mode.DOTS:
		...
		for (int dot = 0; dot < pageCount; dot++) {
			...
			if (dot == _pager.getCurrentItem()) {
				...
				if (_previousPage == -1)
					_previousPage = dot;
				_shrinkFactor = Tool.clampFloat(_shrinkFactor + stepFactor,
				                                smallFactor, largeFactor);
				canvas.drawCircle(_dotSize / 2 + _pad + (_dotSize + _pad * 2) * dot,
				                  (float)(getHeight() / 2),
				                  _shrinkFactor * _dotSize / 2, _paint);
				if (_shrinkFactor != largeFactor)
					invalidate();
			} else if (dot != _pager.getCurrentItem() && dot == _previousPage) {
				// draw expanding dot
				_expandFactor = Tool.clampFloat(_expandFactor - stepFactor,
				                                smallFactor, largeFactor);
				canvas.drawCircle(_dotSize / 2 + _pad + (_dotSize + _pad * 2) * dot,
				                  (float)(getHeight() / 2),
				                  _expandFactor * _dotSize / 2, _paint);
				...
				// draw normal dot
				canvas.drawCircle(_dotSize / 2 + _pad + (_dotSize + _pad * 2) * dot,
				                  (float)(getHeight() / 2), _dotSize / 2, _paint);
			}
		}
		break;
	case Mode.LINES:
