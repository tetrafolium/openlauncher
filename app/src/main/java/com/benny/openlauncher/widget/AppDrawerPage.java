...
private void calculatePage() {
	...
	while ((appsSize = appsSize - (_rowCellCount * _columnCellCount)) >=
	       (_rowCellCount * _columnCellCount) ||
	       (appsSize > -(_rowCellCount * _columnCellCount))) {
		_pageCount++;
	}
}
...
private View getItemView(int page, int x, int y) {
	...
	return ItemViewFactory.getItemView(
		getContext(), null, DragAction.Action.DRAWER, Item.newAppItem(app));
}
...
public Adapter() {
	...
	for (int i = 0; i < getCount(); i++) {
		ViewGroup layout =
			...
			.inflate(R.layout.view_app_drawer_page_inner, null);
		if (!Setup.appSettings().getDrawerShowCardView()) {
			((CardView)layout.getChildAt(0))
			.setCardBackgroundColor(Color.TRANSPARENT);
			((CardView)layout.getChildAt(0)).setCardElevation(0);
		} else {
			((CardView)layout.getChildAt(0))
			.setCardBackgroundColor(Setup.appSettings().getDrawerCardColor());
		...
		for (int x = 0; x < _columnCellCount; x++) {
			for (int y = 0; y < _rowCellCount; y++) {
				...
				if (view != null) {
					CellContainer.LayoutParams lp = new CellContainer.LayoutParams(
						ViewGroup.LayoutParams.MATCH_PARENT,
						ViewGroup.LayoutParams.MATCH_PARENT, x, y, 1, 1);
