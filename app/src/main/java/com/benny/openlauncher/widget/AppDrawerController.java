...
public class AppDrawerController extends RevealFrameLayout {
  ...
  public AppDrawerController(Context context) { super(context); }
  ...
  public void open(int cx, int cy) {
    ...
    _appDrawerAnimator =
        io.codetail.animation.ViewAnimationUtils.createCircularReveal(
            getDrawer(), cx, cy, 0, Math.max(getWidth(), getHeight()));
  ...
  public void close(int cx, int cy) {
    ...
    _appDrawerAnimator =
        io.codetail.animation.ViewAnimationUtils.createCircularReveal(
            getDrawer(), cx, cy, Math.max(getWidth(), getHeight()), 0);
    _appDrawerAnimator.setInterpolator(new AccelerateDecelerateInterpolator());
    _appDrawerAnimator.setDuration(_drawerAnimationTime);
    _appDrawerAnimator.addListener(new Animator.AnimatorListener() {
      @Override
  ...
  public void init() {
    ...
    case Mode.PAGE:
    default:
      _drawerViewPage = (AppDrawerPage)layoutInflater.inflate(
          R.layout.view_app_drawer_page, this, false);
      addView(_drawerViewPage);
      PagerIndicator indicator = (PagerIndicator)layoutInflater.inflate(
          R.layout.view_drawer_indicator, this, false);
      addView(indicator);
