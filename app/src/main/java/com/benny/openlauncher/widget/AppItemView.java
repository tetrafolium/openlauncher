...
public class AppItemView extends View
    implements Drawable.Callback, NotificationListener.NotificationCallback {
  ...
  public AppItemView(Context context) { this(context, null); }
  ...
  public void setIcon(Drawable icon) { _icon = icon; }
  ...
  public float getIconSize() { return _iconSize; }

  public void setIconSize(float iconSize) { _iconSize = iconSize; }

  public boolean getShowLabel() { return _showLabel; }

  public void setTargetedWidth(int width) { _targetedWidth = width; }
  ...
  protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    setMeasuredDimension((int)Math.ceil(mWidth),
                         (int)Math.ceil((int)mHeight) + Tool.dp2px(2) +
                             _targetedHeightPadding * 2);
  }
  ...
  protected void onDraw(Canvas canvas) {
    ...
    _heightPadding =
        (getHeight() - _iconSize - (_showLabel ? _labelHeight : 0)) / 2f;
    ...
    if (_label != null && _showLabel) {
      ...
      if (_textContainer.width() > maxTextWidth) {
        ...
        _textPaint.getTextBounds(testLabel, 0, testLabel.length(),
                                 testTextContainer);

        // Premeditate to be faster
        float characterSize = testTextContainer.width() / testLabel.length();
        int charsToTruncate =
            (int)((testTextContainer.width() - maxTextWidth) / characterSize);

        canvas.drawText(
            _label.substring(0, _label.length() - charsToTruncate) + ELLIPSIS,
            MIN_ICON_TEXT_MARGIN, getHeight() - _heightPadding, _textPaint);
      } else {
        canvas.drawText(_label, (getWidth() - _textContainer.width()) / 2f,
                        getHeight() - _heightPadding, _textPaint);
      }
    }
      ...
      if (_notificationCount > 0) {
        final String count =
            (_notificationCount > 99 ? "++"
                                     : String.valueOf(_notificationCount));
        ...
        canvas.drawText(
            ...
            radius -
                ((_notifyTextPaint.descent() + _notifyTextPaint.ascent()) / 2),
            _notifyTextPaint);
      }
  ...
  public float getDrawIconTop() { return _heightPadding; }

  public float getDrawIconLeft() { return (getWidth() - _iconSize) / 2; }
  ...
  public static class Builder {
    ...
    public Builder(Context context) { _view = new AppItemView(context); }

    public Builder(AppItemView view) { _view = view; }

    public AppItemView getView() { return _view; }
      ...
      _view.setOnClickListener(new OnClickListener() {
        ...
        public void onClick(View v) {
          Tool.createScaleInScaleOutAnim(_view, new Runnable() {
            @Override
            public void run() {
              Tool.startApp(_view.getContext(),
                            AppManager.getInstance(_view.getContext())
                                .findApp(item._intent),
                            _view);
            }
          });
        }
    ...
    public Builder setGroupItem(Context context, final DesktopCallback callback,
                                final Item item) {
      _view.setLabel(item.getLabel());
      _view.setIcon(
          new GroupDrawable(context, item, Setup.appSettings().getIconSize()));
    ...
    public Builder setActionItem(Item item) {
      _view.setLabel(item.getLabel());
      _view.setIcon(ContextCompat.getDrawable(Setup.appContext(),
                                              R.drawable.item_drawer));
      _view.setOnClickListener(new OnClickListener() {
        @Override
    ...
    public Builder withOnLongClick(final Item item,
                                   final DragAction.Action action,
                                   DesktopCallback desktopCallback) {
      _view.setOnLongClickListener(
          DragHandler.getLongClick(item, action, desktopCallback));
      return this;
    }
