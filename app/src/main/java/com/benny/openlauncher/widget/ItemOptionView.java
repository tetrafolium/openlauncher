...
public final class ItemOptionView extends FrameLayout {
  ...
  @Nullable private Action _dragAction;
  private boolean _dragExceedThreshold;
  @Nullable private Item _dragItem;
  @NonNull private PointF _dragLocation;
  private PointF _dragLocationConverted;
  private PointF _dragLocationStart;
  @Nullable private View _dragView;
  ...
  private final HashMap<DropTargetListener, DragFlag>
      _registeredDropTargetEntries;
  ...
  private PopupIconLabelItem uninstallItem =
      new PopupIconLabelItem(R.string.uninstall, R.drawable.ic_delete)
          .withIdentifier(uninstallItemIdentifier);
  private PopupIconLabelItem infoItem =
      new PopupIconLabelItem(R.string.info, R.drawable.ic_info)
          .withIdentifier(infoItemIdentifier);
  private PopupIconLabelItem editItem =
      new PopupIconLabelItem(R.string.edit, R.drawable.ic_edit)
          .withIdentifier(editItemIdentifier);
  private PopupIconLabelItem removeItem =
      new PopupIconLabelItem(R.string.remove, R.drawable.ic_close)
          .withIdentifier(removeItemIdentifier);
  private PopupIconLabelItem resizeItem =
      new PopupIconLabelItem(R.string.resize, R.drawable.ic_resize)
          .withIdentifier(resizeItemIdentifier);

  public static final class DragFlag {
    private boolean _previousOutside = true;
    private boolean _shouldIgnore;

    public final boolean getPreviousOutside() { return _previousOutside; }

    public final void setPreviousOutside(boolean v) { _previousOutside = v; }

    public final boolean getShouldIgnore() { return _shouldIgnore; }

    public final void setShouldIgnore(boolean v) { _shouldIgnore = v; }
  }
  ...
  public final class OverlayView extends View {
    ...
    public boolean onTouchEvent(@Nullable MotionEvent event) {
      if (event == null || event.getActionMasked() != 0 ||
          ItemOptionView.this.getDragging() ||
          !ItemOptionView.this._overlayPopupShowing) {
    ...
    protected void onDraw(@Nullable Canvas canvas) {
      super.onDraw(canvas);
      if (canvas == null || DragHandler._cachedDragBitmap == null ||
          _dragLocation.equals(-1f, -1f))
        return;
      ...
      if (_dragging) {
        canvas.save();
        _overlayIconScale =
            Tool.clampFloat(_overlayIconScale + 0.05f, 1f, 1.1f);
        canvas.scale(_overlayIconScale, _overlayIconScale,
                     x + DragHandler._cachedDragBitmap.getWidth() / 2,
                     y + DragHandler._cachedDragBitmap.getHeight() / 2);
        canvas.drawBitmap(DragHandler._cachedDragBitmap,
                          x - DragHandler._cachedDragBitmap.getWidth() / 2,
                          y - DragHandler._cachedDragBitmap.getHeight() / 2,
                          _paint);
        canvas.restore();
  ...
  public ItemOptionView(@NonNull Context context,
    ...
    _slideInLeftAnimator =
        new SlideInLeftAnimator(new AccelerateDecelerateInterpolator());
    _slideInRightAnimator =
        new SlideInRightAnimator(new AccelerateDecelerateInterpolator());
  ...
  public final boolean getDragging() { return _dragging; }

  public final PointF getDragLocation() { return _dragLocation; }

  public final Action getDragAction() { return _dragAction; }

  public final boolean getDragExceedThreshold() { return _dragExceedThreshold; }
  ...
  public final void showFolderPreviewAt(@NonNull View fromView, float x,
                                        float y) {
  ...
  public final void convertPoint(@NonNull View fromView, @NonNull View toView,
                                 float x, float y) {
    ...
    _previewLocation.set(fromCoordinate[0] - toCoordinate[0] + x,
  ...
  protected void onDraw(@Nullable Canvas canvas) {
    super.onDraw(canvas);
    if (canvas != null && _showFolderPreview &&
        !_previewLocation.equals(-1.0f, -1.0f)) {
      _folderPreviewScale += 0.08f;
      _folderPreviewScale = Tool.clampFloat(_folderPreviewScale, 0.5f, 1.0f);
      canvas.drawCircle(
          _previewLocation.x, _previewLocation.y,
          ((float)Tool.dp2px((Setup.appSettings().getDesktopIconSize() / 2) +
                             10)) *
              _folderPreviewScale,
          _paint);
    }
  ...
  public final void
  showPopupMenuForItem(float x, float y,
                       @NonNull List<PopupIconLabelItem> popupItem,
                       com.mikepenz.fastadapter.listeners
                           .OnClickListener<PopupIconLabelItem> listener) {
  ...
  public final void startDragNDropOverlay(@NonNull View view,
                                          @NonNull Item item,
    ...
    for (Entry dropTarget : _registeredDropTargetEntries.entrySet()) {
      ...
      dragFlag.setShouldIgnore(!dropTargetListener.onStart(
          _dragAction, _dragLocationConverted,
          isViewContains(((DropTargetListener)dropTarget.getKey()).getView(),
  ...
  public final void
  registerDropTarget(@NonNull DropTargetListener targetListener) {
  ...
  public void showItemPopup(final HomeActivity homeActivity) {
    ...
    float y = getDragLocation().y - HomeActivity._itemTouchY -
              Tool.dp2px((46 * itemList.size()));

    if ((x + Tool.dp2px(200)) > getWidth()) {
      setPopupMenuShowDirection(false);
      x = getDragLocation().x - HomeActivity._itemTouchX +
          homeActivity.getDesktop().getCurrentPage().getCellWidth() -
          Tool.dp2px(200) - Tool.dp2px(10);
    } else {
      setPopupMenuShowDirection(true);
    }

    if (y < 0) {
      y = getDragLocation().y - HomeActivity._itemTouchY +
          homeActivity.getDesktop().getCurrentPage().getCellHeight() +
          Tool.dp2px(4);
    } else {
      y -= Tool.dp2px(4);
    }

    showPopupMenuForItem(x, y, itemList,
                         new com.mikepenz.fastadapter.listeners
                             .OnClickListener<PopupIconLabelItem>() {
                               @Override
                               public boolean onClick(
                                   View v, IAdapter<PopupIconLabelItem> adapter,
                                   PopupIconLabelItem item, int position) {
                                 Item dragItem = getDragItem();
                                 if (dragItem != null) {
                                   HpItemOption itemOption =
                                       new HpItemOption(homeActivity);
  ...
  private final void handleMovement() {
    if (!_dragExceedThreshold &&
        (Math.abs(_dragLocationStart.x - _dragLocation.x) >
             this.DRAG_THRESHOLD ||
         Math.abs(_dragLocationStart.y - _dragLocation.y) >
             this.DRAG_THRESHOLD)) {
    ...
    for (Entry<DropTargetListener, DragFlag> dropTarget2 :
         _registeredDropTargetEntries.entrySet()) {
      ...
      if (!dropTarget2.getValue().getShouldIgnore()) {
        ...
        if (isViewContains(dropTarget2.getKey().getView(), (int)_dragLocation.x,
                           (int)_dragLocation.y)) {
  ...
  public void convertPoint(@NonNull View toView) {
    ...
    _dragLocationConverted.set(
        ((float)(fromCoordinate[0] - toCoordinate[0])) + _dragLocation.x,
        ((float)(fromCoordinate[1] - toCoordinate[1])) + _dragLocation.y);
  }
