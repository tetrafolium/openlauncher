...
public class DesktopOptionView extends FrameLayout {
  ...
  public void setDesktopOptionViewListener(
      DesktopOptionViewListener desktopOptionViewListener) {
  ...
  public void updateHomeIcon(final boolean home) {
    post(new Runnable() {
      ...
      public void run() {
        if (home) {
          _actionAdapters[0].getAdapterItem(1)._icon =
              getContext().getResources().getDrawable(R.drawable.ic_star);
        } else {
          _actionAdapters[0].getAdapterItem(1)._icon =
  ...
  public void updateLockIcon(final boolean lock) {
    if (_actionAdapters.length == 0)
      return;
    if (_actionAdapters[0].getAdapterItemCount() == 0)
      return;
    post(new Runnable() {
      @Override
      public void run() {
        if (lock) {
          _actionAdapters[0].getAdapterItem(2)._icon =
              getContext().getResources().getDrawable(R.drawable.ic_lock);
        } else {
          _actionAdapters[0].getAdapterItem(2)._icon =
              getContext().getResources().getDrawable(R.drawable.ic_lock_open);
        }
  ...
  private void init() {
    ...
    final Typeface typeface = Typeface.createFromAsset(
        getContext().getAssets(), "RobotoCondensed-Regular.ttf");

    _actionAdapters[0] = new FastItemAdapter<>();
    _actionAdapters[1] = new FastItemAdapter<>();

    _actionRecyclerViews[0] = createRecyclerView(
        _actionAdapters[0], Gravity.TOP | Gravity.CENTER_HORIZONTAL,
        paddingHorizontal);
    _actionRecyclerViews[1] = createRecyclerView(
        _actionAdapters[1], Gravity.BOTTOM | Gravity.CENTER_HORIZONTAL,
        paddingHorizontal);

    final com.mikepenz.fastadapter.listeners
        .OnClickListener<IconLabelItem> clickListener =
        new com.mikepenz.fastadapter.listeners
            .OnClickListener<IconLabelItem>() {
              @Override
              public boolean onClick(View v, IAdapter<IconLabelItem> adapter,
                                     IconLabelItem item, int position) {
                if (_desktopOptionViewListener != null) {
                  ...
                  if (id == R.string.home) {
                    ...
                    Setup.appSettings().setDesktopLock(
                        !Setup.appSettings().getDesktopLock());
  ...
  private void initItems(final Typeface typeface,
    ...
    itemsTop.add(
        createItem(R.drawable.ic_delete, R.string.remove, typeface, itemWidth));
    itemsTop.add(
        createItem(R.drawable.ic_star, R.string.home, typeface, itemWidth));
    itemsTop.add(
        createItem(R.drawable.ic_lock, R.string.lock, typeface, itemWidth));
    ...
    itemsBottom.add(createItem(R.drawable.ic_dashboard, R.string.widget,
                               typeface, itemWidth));
    itemsBottom.add(
        createItem(R.drawable.ic_launch, R.string.action, typeface, itemWidth));
    itemsBottom.add(createItem(R.drawable.ic_settings,
                               R.string.pref_title__settings, typeface,
                               itemWidth));
    _actionAdapters[1].set(itemsBottom);
    _actionAdapters[1].withOnClickListener(clickListener);

    ((MarginLayoutParams)((View)_actionRecyclerViews[0].getParent())
         .getLayoutParams())
        .topMargin =
        Tool.dp2px(Setup.appSettings().getSearchBarEnable() ? 36 : 4);
  }
  ...
  private RecyclerView createRecyclerView(FastAdapter adapter, int gravity,
    ...
    LayoutParams actionRecyclerViewLP =
  ...
  private IconLabelItem createItem(int icon, int label, Typeface typeface,
                                   int width) {
