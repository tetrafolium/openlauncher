...
public class CellContainer extends ViewGroup {
  ...
  @NonNull private final Rect _tempRect = new Rect();
  ...
  public static final class LayoutParams
    ...
    public final int getXSpan() { return _xSpan; }

    public final void setXSpan(int v) { _xSpan = v; }

    public final int getYSpan() { return _ySpan; }

    public final void setYSpan(int v) { _ySpan = v; }
  ...
  public enum PeekDirection { UP, LEFT, RIGHT, DOWN }

  public final int getCellWidth() { return _cellWidth; }

  public final int getCellHeight() { return _cellHeight; }

  public final int getCellSpanV() { return _cellSpanV; }

  public final int getCellSpanH() { return _cellSpanH; }

  public final void setBlockTouch(boolean v) { _blockTouch = v; }
  ...
  public CellContainer(Context context) { this(context, null); }
  ...
  public final void projectImageOutlineAt(@NonNull Point newCoordinate,
  ...
  private void drawCachedOutlineBitmap(Canvas canvas, Rect cell) {
    if (_cachedOutlineBitmap != null) {
      ...
      canvas.drawBitmap(bitmap, centerX - (bitmap.getWidth() / 2),
                        centerY - (bitmap.getHeight() / 2), _outlinePaint);
    }
  }
  ...
  @NonNull
  public final DragState peekItemAndSwap(@NonNull DragEvent event,
                                         @NonNull Point coordinate) {
  ...
  public final DragState peekItemAndSwap(int x, int y, Point coordinate) {
    ...
    if (coordinate.x != -1 && coordinate.y != -1) {
      ...
      if (_peekDownTime != null && _peekDownTime == -1) {
        _peekDirection =
            getPeekDirectionFromCoordinate(_startCoordinate, coordinate);
  ...
  protected void onDraw(Canvas canvas) {
    ...
    for (int x = 0; x < _cellSpanH; x++) {
      for (int y = 0; y < _cellSpanV; y++) {
        ...
        canvas.drawRect(cell.left - s, cell.top - s, cell.left + s,
                        cell.top + s, _paint);
        canvas.restore();

        canvas.save();
        canvas.rotate(45f, cell.left, cell.bottom);
        canvas.drawRect(cell.left - s, cell.bottom - s, cell.left + s,
                        cell.bottom + s, _paint);
        canvas.restore();

        canvas.save();
        canvas.rotate(45f, cell.right, cell.top);
        canvas.drawRect(cell.right - s, cell.top - s, cell.right + s,
                        cell.top + s, _paint);
        canvas.restore();

        canvas.save();
        canvas.rotate(45f, cell.right, cell.bottom);
        canvas.drawRect(cell.right - s, cell.bottom - s, cell.right + s,
                        cell.bottom + s, _paint);
        canvas.restore();
      }
    }
    ...
    if (homeActivity != null &&
        homeActivity.getItemOptionView().getDragExceedThreshold() &&
        _currentOutlineCoordinate.x != -1 &&
        _currentOutlineCoordinate.y != -1) {
      if (_outlinePaint.getAlpha() != 160)
        _outlinePaint.setAlpha(Math.min(_outlinePaint.getAlpha() + 20, 160));
      drawCachedOutlineBitmap(
          canvas,
          _cells[_currentOutlineCoordinate.x][_currentOutlineCoordinate.y]);
  ...
  public final void addViewToGrid(@NonNull View view, int x, int y, int xSpan,
                                  int ySpan) {
    view.setLayoutParams(
        new LayoutParams(WRAP_CONTENT, WRAP_CONTENT, x, y, xSpan, ySpan));
    addView(view);
  }

  public final void addViewToGrid(@NonNull View view) { addView(view); }
  ...
  public final View coordinateToChildView(Point pos) {
    ...
    for (int i = 0; i < getChildCount(); i++) {
      ...
      if (pos.x >= lp._x && pos.y >= lp._y && pos.x < lp._x + lp._xSpan &&
          pos.y < lp._y + lp._ySpan) {
        return getChildAt(i);
      }
    }
    return null;
  ...
  public final LayoutParams coordinateToLayoutParams(int mX, int mY, int xSpan,
                                                     int ySpan) {
    Point pos = new Point();
    touchPosToCoordinate(pos, mX, mY, xSpan, ySpan, true);
    return !pos.equals(-1, -1) ? new LayoutParams(WRAP_CONTENT, WRAP_CONTENT,
                                                  pos.x, pos.y, xSpan, ySpan)
                               : null;
  }

  public void touchPosToCoordinate(@NonNull Point coordinate, int mX, int mY,
                                   int xSpan, int ySpan,
                                   boolean checkAvailability) {
    touchPosToCoordinate(coordinate, mX, mY, xSpan, ySpan, checkAvailability,
                         false);
  }

  public final void touchPosToCoordinate(Point coordinate, int mX, int mY,
                                         int xSpan, int ySpan,
                                         boolean checkAvailability,
    ...
    while (x < _cellSpanH) {
      ...
      while (y < _cellSpanV) {
        Rect cell = _cells[x][y];
        if (mY >= cell.top && mY <= cell.bottom && mX >= cell.left &&
            mX <= cell.right) {
          ...
          if (checkBoundary) {
            ...
            if (mY >= offsetCell.top && mY <= offsetCell.bottom &&
                mX >= offsetCell.left && mX <= offsetCell.right) {
              coordinate.set(-1, -1);
              return;
            }
          }
  ...
  protected void onLayout(boolean changed, int l, int t, int r, int b) {
    ...
    initCellInfo(getPaddingLeft(), getPaddingTop(), width - getPaddingRight(),
    ...
    if (_cells != null) {
      ...
      while (i < count) {
        ...
        if (child.getVisibility() != View.GONE) {
          ...
          child.measure(MeasureSpec.makeMeasureSpec(lp.getXSpan() * _cellWidth,
                                                    View.MeasureSpec.EXACTLY),
                        MeasureSpec.makeMeasureSpec(lp.getYSpan() * _cellHeight,
                                                    View.MeasureSpec.EXACTLY));
          Rect[][] rectArr = _cells;
          Rect upRect = rectArr[lp.getX()][lp.getY()];
          Rect downRect = _tempRect;
          if ((lp.getX() + lp.getXSpan()) - 1 < _cellSpanH &&
              (lp.getY() + lp.getYSpan()) - 1 < _cellSpanV) {
            Rect[][] rectArr2 = _cells;
            downRect = rectArr2[(lp.getX() + lp.getXSpan()) - 1]
                               [(lp.getY() + lp.getYSpan()) - 1];
          }
          if (lp.getXSpan() == 1 && lp.getYSpan() == 1) {
            ...
            child.layout(upRect.left, upRect.top, downRect.right,
                         downRect.bottom);
          } else if (lp.getXSpan() > 1) {
            child.layout(upRect.left, upRect.top, downRect.right,
                         upRect.bottom);
          } else if (lp.getYSpan() > 1) {
            child.layout(upRect.left, upRect.top, upRect.right,
                         downRect.bottom);
          }
        }
