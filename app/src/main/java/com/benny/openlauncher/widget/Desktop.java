...
public final class Desktop extends ViewPager implements DesktopCallback {
  ...
  public Desktop(Context context) { super(context, null); }

  public Desktop(Context context, AttributeSet attr) { super(context, attr); }

  public static boolean handleOnDropOver(HomeActivity homeActivity,
  ...
  public final class DesktopAdapter extends PagerAdapter {
    ...
    private OnFingerGestureListener getGestureListener() {
      return new DesktopGestureListener(_desktop,
                                        Setup.desktopGestureCallback());
    }
    ...
    private CellContainer getItemLayout() {
      ...
      layout.setGridSize(Setup.appSettings().getDesktopColumnCount(),
                         Setup.appSettings().getDesktopRowCount());
    ...
    private void enterDesktopEditMode() {
      ...
      float translateFactor =
          (float)Tool.dp2px(Setup.appSettings().getSearchBarEnable() ? 20 : 40);
      for (CellContainer v : _desktop.getPages()) {
        ...
        ViewPropertyAnimator animation = v.animate()
                                             .scaleX(scaleFactor)
                                             .scaleY(scaleFactor)
                                             .translationY(translateFactor);
        animation.setInterpolator(new AccelerateDecelerateInterpolator());
      }
      _desktop.setInEditMode(true);
      if (_desktop.getDesktopEditListener() != null) {
        OnDesktopEditListener desktopEditListener =
            _desktop.getDesktopEditListener();
  ...
  public final List<CellContainer> getPages() { return _pages; }
  ...
  public final boolean getInEditMode() { return _inEditMode; }

  public final void setInEditMode(boolean v) { _inEditMode = v; }
  ...
  public final void initDesktop() {
    ...
    if (Setup.appSettings().getDesktopShowIndicator() &&
        _pageIndicator != null) {
  ...
  public final void updateIconProjection(int x, int y) {
    ...
    case CurrentNotOccupied:
      getCurrentPage().projectImageOutlineAt(_coordinate,
    ...
    case CurrentOccupied:
      ...
      if (!type.equals(Type.WIDGET) &&
          (getCurrentPage().coordinateToChildView(_coordinate) instanceof
           AppItemView)) {
        dragNDropView.showFolderPreviewAt(
            this, getCurrentPage().getCellWidth() * (_coordinate.x + 0.5f),
            getCurrentPage().getCellHeight() * (_coordinate.y + 0.5f));
      }
      break;
    case OutOffRange:
  ...
  public boolean addItemToPage(@NonNull Item item, int page) {
    View itemView =
        ItemViewFactory.getItemView(getContext(), this, Action.DESKTOP, item);
    if (itemView == null) {
    ...
    _pages.get(page).addViewToGrid(itemView, item._x, item._y, item._spanX,
                                   item._spanY);
    return true;
  }

  public boolean addItemToPoint(@NonNull Item item, int x, int y) {
    CellContainer.LayoutParams positionToLayoutPrams =
        getCurrentPage().coordinateToLayoutParams(x, y, item._spanX,
                                                  item._spanY);
  ...
  public boolean addItemToCell(@NonNull Item item, int x, int y) {
    ...
    getCurrentPage().addViewToGrid(itemView, item._x, item._y, item._spanX,
                                   item._spanY);
    return true;
  }
  ...
  protected void onPageScrolled(int position, float offset, int offsetPixels) {
    Definitions.WallpaperScroll scroll =
        Setup.appSettings().getDesktopWallpaperScroll();
