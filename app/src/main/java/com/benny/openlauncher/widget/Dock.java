...
public final class Dock extends CellContainer implements DesktopCallback {
  ...
  public Dock(Context context, AttributeSet attr) { super(context, attr); }
  ...
  private void detectSwipe(MotionEvent ev) {
    ...
    case 0:
      ...
      break;
    case 1:
      if (_startPosY - ev.getY() > 150.0f &&
          Setup.appSettings().getGestureDockSwipeUp()) {
        Point point = new Point((int)ev.getX(), (int)ev.getY());
        point = Tool.convertPoint(point, this,
                                  _homeActivity.getAppDrawerController());
  ...
  public final void updateIconProjection(int x, int y) {
    ...
    case CurrentOccupied:
      ...
      if (!type.equals(Item.Type.WIDGET) &&
          (coordinateToChildView(_coordinate) instanceof AppItemView)) {
        dragNDropView.showFolderPreviewAt(
            this, getCellWidth() * (_coordinate.x + 0.5f),
            getCellHeight() * (_coordinate.y + 0.5f) -
                (Setup.appSettings().getDockShowLabel() ? Tool.dp2px(7) : 0));
      }
      break;
    case OutOffRange:
  ...
  protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    if (!isInEditMode()) {
      // set the height for the dock based on the number of rows and the show
      // label preference
  ...
  public boolean addItemToPoint(@NonNull Item item, int x, int y) {
    LayoutParams positionToLayoutPrams =
        coordinateToLayoutParams(x, y, item._spanX, item._spanY);
