...
private void init() {
	...
	_popupCard = (CardView)LayoutInflater.from(getContext())
	             .inflate(R.layout.view_group_popup, this, false);
	// set the CardView color
	int color = Setup.appSettings().getDesktopFolderColor();
	int alpha = Color.alpha(color);
	_popupCard.setCardBackgroundColor(color);
	// remove elevation if CardView's background is transparent to avoid weird
	// shadows because CardView does not support transparent backgrounds
	if (alpha == 0) {
...
public boolean showPopup(final Item item, final View itemView,
                         final DesktopCallback callback) {
	if (_isShowing || getVisibility() == View.VISIBLE)
		return false;
	_isShowing = true;
	...
	_textViewGroupName.setTextColor(
		cu.shouldColorOnTopBeLight(Setup.appSettings().getDesktopFolderColor())
	    ? Color.WHITE
	    : Color.BLACK);
	...
	int[] cellSize =
		GroupPopupView.GroupDef.getCellSize(item.getGroupItems().size());
	...
	for (int x2 = 0; x2 < cellSize[0]; x2++) {
		for (int y2 = 0; y2 < cellSize[1]; y2++) {
			...
			if (app == null || AppSettings.get().getHiddenAppsList().contains(
				    app.getComponentName())) {
				deleteItem(context, item, groupItem, (AppItemView)itemView);
				appsChanged = true;
				continue;
			} else {
				final View view = ItemViewFactory.getItemView(
					getContext(), callback, DragAction.Action.DESKTOP, groupItem);
				view.setOnLongClickListener(new OnLongClickListener() {
						...
						public boolean onLongClick(View view2) {
						        ...
						        DragHandler.startDrag(view, groupItem, DragAction.Action.DESKTOP,
						                              null);
	...
	int popupWidth = contentPadding * 8 + _popupCard.getContentPaddingLeft() +
	                 _popupCard.getContentPaddingRight() +
	                 (iconSize)*cellSize[0];
	_popupCard.getLayoutParams().width = popupWidth;

	int popupHeight = contentPadding * 2 + _popupCard.getContentPaddingTop() +
	                  _popupCard.getContentPaddingBottom() + Tool.dp2px(30) +
	                  (iconSize + textSize) * cellSize[1];
	_popupCard.getLayoutParams().height = popupHeight;

	_cx = popupWidth / 2;
	_cy = popupHeight / 2 -
	      (Setup.appSettings().getDesktopShowLabel() ? Tool.dp2px(10) : 0);
	...
	if (item._location == ItemPosition.Dock) {
		...
		_cy += iconSize / 2 +
		       (Setup.appSettings().getDockShowLabel() ? 0 : Tool.dp2px(10));
	}
	...
	if (appsChanged) {
		updateItem(callback, item, itemView);
		Toast
		.makeText(context, R.string.toast_update_group_due_to_missing_items,
		          Toast.LENGTH_LONG)
		.show();
	}
...
private void expand() {
	...
	_folderAnimator = ViewAnimationUtils.createCircularReveal(
		_popupCard, _cx, _cy, startRadius, finalRadius);
...
public void collapse() {
	...
	int finalRadius = Math.max(_popupCard.getWidth(), _popupCard.getHeight());
	_folderAnimator = ViewAnimationUtils.createCircularReveal(
		_popupCard, _cx, _cy, finalRadius, startRadius);
...
private void removeItem(Context context, final Item currentItem,
                        Item dragOutItem, AppItemView currentView) {
	...
	currentView.setIcon(new GroupDrawable(
				    context, currentItem, Setup.appSettings().getDesktopIconSize()));
}

private void deleteItem(Context context, final Item currentItem,
                        Item dragOutItem, AppItemView currentView) {
	currentItem.getGroupItems().remove(dragOutItem);
...
public void updateItem(DesktopCallback callback, final Item currentItem,
                       View currentView) {
	if (currentItem.getGroupItems().size() == 1) {
		final App app =
			Setup.appLoader().findItemApp(currentItem.getGroupItems().get(0));
		if (app != null) {
			Item item = HomeActivity._db.getItem(
				currentItem.getGroupItems().get(0).getId());
			...
			HomeActivity._db.saveItem(
				item, HomeActivity._launcher.getDesktop().getCurrentItem(),
		...
		callback.removeItem(currentView, false);
		callback.addItemToCell(currentItem, currentItem.getX(),
		                       currentItem.getY());
	}
}
