	...
	for (int i = 0; i < 4 && i < item.getItems().size(); i++) {
		...
		if (app == null) {
			Log.d(this.getClass().getName(),
			      String.format("Item %s has a null app at index %d (Intent: %s)",
			                    item.getLabel(), i,
			                    temp == null ? "Item is NULL" : temp.getIntent()));
...
private static final float PADDING32_KOEF =
	((float)(Math.sqrt(3) - 1f) / (2f * (float)Math.sqrt(3)));
...
public void draw(Canvas canvas) {
	...
	Path clip = new Path();
	clip.addCircle(_iconSize / 2, _iconSize / 2, _iconSize / 2 - _outline,
	               Path.Direction.CW);
	...
	canvas.drawCircle(_iconSize / 2, _iconSize / 2, _iconSize / 2 - _outline,
	                  _paintInnerCircle);

	if (_iconsCount > 3) {
		if (_icons[0] != null) {
			drawIcon(canvas, _icons[0], _padding, _padding,
			         _iconSizeDiv2 - _padding, _iconSizeDiv2 - _padding,
			         _paintIcon);
		}
		if (_icons[1] != null) {
			drawIcon(canvas, _icons[1], _iconSizeDiv2 + _padding, _padding,
			         _iconSize - _padding, _iconSizeDiv2 - _padding, _paintIcon);
		}
		if (_icons[2] != null) {
			drawIcon(canvas, _icons[2], _padding, _iconSizeDiv2 + _padding,
			         _iconSizeDiv2 - _padding, _iconSize - _padding, _paintIcon);
		}
		if (_icons[3] != null) {
			drawIcon(canvas, _icons[3], _iconSizeDiv2 + _padding,
			         _iconSizeDiv2 + _padding, _iconSize - _padding,
			         _iconSize - _padding, _paintIcon);
		}
	} else if (_iconsCount > 2) {
		if (_icons[0] != null) {
			drawIcon(canvas, _icons[0], _padding, _padding + _padding31,
			         _iconSizeDiv2 - _padding,
			         _iconSizeDiv2 - _padding + _padding31, _paintIcon);
		}
		if (_icons[1] != null) {
			drawIcon(canvas, _icons[1], _iconSizeDiv2 + _padding,
			         _padding + _padding31, _iconSize - _padding,
			         _iconSizeDiv2 - _padding + _padding31, _paintIcon);
		}
		if (_icons[2] != null) {
			drawIcon(canvas, _icons[2], _padding + _iconSizeDiv4,
			         _iconSizeDiv2 + _padding + _padding32,
			         _iconSizeDiv4 + _iconSizeDiv2 - _padding,
			         _iconSize - _padding + _padding32, _paintIcon);
		}
	} else { // if (_iconsCount <= 2) {
		if (_icons[0] != null) {
			drawIcon(canvas, _icons[0], _padding, _padding + _iconSizeDiv4,
			         _iconSizeDiv2 - _padding,
			         _iconSizeDiv4 + _iconSizeDiv2 - _padding, _paintIcon);
		}
		if (_icons[1] != null) {
			drawIcon(canvas, _icons[1], _iconSizeDiv2 + _padding,
			         _padding + _iconSizeDiv4, _iconSize - _padding,
			         _iconSizeDiv4 + _iconSizeDiv2 - _padding, _paintIcon);
		}
	}
	if (Build.VERSION.SDK_INT < Build.VERSION_CODES.P) {
...
private void drawIcon(Canvas canvas, Drawable icon, float l, float t, float r,
                      float b, Paint paint) {
