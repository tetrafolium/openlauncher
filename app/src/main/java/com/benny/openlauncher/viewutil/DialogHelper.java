...
public static void editItemDialog(String title, String defaultText, Context c,
                                  final OnItemEditListener listener) {
	MaterialDialog.Builder builder = new MaterialDialog.Builder(c);
	builder.title(title)
	.positiveText(android.R.string.ok)
	.negativeText(android.R.string.cancel)
	.input(null, defaultText,
...
public static void
alertDialog(Context context, String title, String msg,
...
public static void
alertDialog(Context context, String title, String message, String positive,
            MaterialDialog.SingleButtonCallback onPositive) {
	MaterialDialog.Builder builder = new MaterialDialog.Builder(context);
	builder.title(title)
	.onPositive(onPositive)
...
public static void selectActionDialog(final Context context,
...
public static void
selectDesktopActionDialog(final Context context,
                          MaterialDialog.ListCallback callback) {
	MaterialDialog.Builder builder = new MaterialDialog.Builder(context);
	builder.title(R.string.action)
...
public static void selectGestureDialog(final Context context, String title,
                                       MaterialDialog.ListCallback callback) {
	MaterialDialog.Builder builder = new MaterialDialog.Builder(context);
	builder.title(title)
...
public static void
selectAppDialog(final Context context,
                final OnAppSelectedListener onAppSelectedListener) {
	MaterialDialog.Builder builder = new MaterialDialog.Builder(context);
	...
	fastItemAdapter.withOnClickListener(
...
public static void startPickIconPackIntent(final Context context) {
	...
	fastItemAdapter.add(new IconLabelItem(activity, R.mipmap.ic_launcher,
	                                      R.string.default_icons)
	                    .withIconPadding(16)
	                    .withIconGravity(Gravity.START)
	...
	for (int i = 0; i < resolveInfos.size(); i++) {
		...
		fastItemAdapter.add(
			new IconLabelItem(
				resolveInfos.get(i).loadIcon(packageManager),
				resolveInfos.get(i).loadLabel(packageManager).toString())
			.withIconPadding(16)
			.withIconSize(50)
			.withIsAppLauncher(true)
			.withIconGravity(Gravity.START)
				...
				public void onClick(View v) {
				        if (ActivityCompat.checkSelfPermission(
				            ...
				            PackageManager.PERMISSION_GRANTED) {
				                appManager._recreateAfterGettingApps = true;
				                AppSettings.get().setIconPack(
							resolveInfos.get(mI).activityInfo.packageName);
				                appManager.getAllApps();
				                dialog.dismiss();
					} else {
				                Tool.toast(context, (activity.getString(
									     R.string.toast_icon_pack_error)));
	...
	if (item.getType() == Item.Type.APP) {
		try {
			Uri packageURI = Uri.parse(
				"package:" + item.getIntent().getComponent().getPackageName());
...
public interface OnAppSelectedListener { void onAppSelected(App app); }

public interface OnItemEditListener { void itemLabel(String label); }
}
