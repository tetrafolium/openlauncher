...
public static View getItemView(final Context context,
                               final DesktopCallback callback,
                               final DragAction.Action type,
                               final Item item) {
	View view = null;
...
public static View getWidgetView(final Context context,
                                 final DesktopCallback callback,
                                 final DragAction.Action type,
                                 final Item item) {
	...
	AppWidgetProviderInfo appWidgetInfo =
		HomeActivity._appWidgetManager.getAppWidgetInfo(item.getWidgetValue());

	// If we can't find the Widget, we don't want to proceed or we'll end up
	// with a phantom on the home screen.
	if (appWidgetInfo == null) {
		...
		if (item._label.contains(Definitions.DELIMITER)) {
			...
			if (HomeActivity._appWidgetManager.bindAppWidgetIdIfAllowed(appWidgetId,
			                                                            cn)) {
				appWidgetInfo =
					HomeActivity._appWidgetManager.getAppWidgetInfo(appWidgetId);
				...
				HomeActivity._launcher.startActivityForResult(
					intent, HomeActivity.REQUEST_PICK_APPWIDGET);
				return null;
			}
		} else {
			// Delete the Widget if we don't have enough information to rehydrate
			// it.
			LOG.debug(
				"Unable to identify Widget for rehydration; removing from database");
	...
	final WidgetView widgetView =
		(WidgetView)HomeActivity._appWidgetHost.createView(
			context, item.getWidgetValue(), appWidgetInfo);
	widgetView.setAppWidget(item.getWidgetValue(), appWidgetInfo);

	final WidgetContainer widgetContainer =
		new WidgetContainer(context, widgetView, item);
	...
	widgetView.setOnLongClickListener(new View.OnLongClickListener() {
			...
			public boolean onLongClick(View view) {
			        ...
			        DragHandler.startDrag(widgetContainer, item, DragAction.Action.DESKTOP,
			                              callback);
			        return true;
			}
		});
