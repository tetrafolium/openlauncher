...
public class AppManager {
  ...
  public static AppManager getInstance(Context context) {
    return appManager == null ? (appManager = new AppManager(context))
                              : appManager;
  }
  ...
  public PackageManager getPackageManager() { return _packageManager; }
  ...
  public App findApp(Intent intent) {
    ...
    for (App app : _apps) {
      if (app._className.equals(className) &&
          app._packageName.equals(packageName)) {
        return app;
      }
    }
    return null;
  ...
  public List<App> getNonFilteredApps() { return _nonFilteredApps; }

  public void init() { getAllApps(); }
  ...
  public App findItemApp(Item item) { return findApp(item.getIntent()); }
  ...
  public void onAppUpdated(Context context, Intent intent) { getAllApps(); }
  ...
  private class AsyncGetApps extends AsyncTask {
    ...
    protected Object doInBackground(Object[] p1) {
      ...
      if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
        LauncherApps launcherApps = (LauncherApps)_context.getSystemService(
        ...
        for (UserHandle userHandle : profiles) {
          ...
          for (LauncherActivityInfo info : apps) {
            ...
            LOG.debug("adding work profile to non filtered list: {}, {}, {}",
                      app._label, app._packageName, app._className);
        ...
        List<ResolveInfo> activitiesInfo =
            _packageManager.queryIntentActivities(intent, 0);
        for (ResolveInfo info : activitiesInfo) {
          App app = new App(_packageManager, info);

          LOG.debug("adding app to non filtered list: {}, {}, {}", app._label,
                    app._packageName, app._className);
          nonFilteredAppsTemp.add(app);
        }
      }
      ...
      if (hiddenList != null) {
        for (int i = 0; i < nonFilteredAppsTemp.size(); i++) {
          ...
          for (String hidItemRaw : hiddenList) {
            if ((nonFilteredAppsTemp.get(i).getComponentName())
                    .equals(hidItemRaw)) {
              shouldGetAway = true;
              break;
            }
      ...
      if (!appSettings.getIconPack().isEmpty() &&
          Tool.isPackageInstalled(appSettings.getIconPack(), _packageManager)) {
        IconPackHelper.applyIconPack(AppManager.this,
                                     Tool.dp2px(appSettings.getIconSize()),
                                     appSettings.getIconPack(), appsTemp);
      }
      return null;
    }
