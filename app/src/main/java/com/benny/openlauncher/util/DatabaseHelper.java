...
public class DatabaseHelper extends SQLiteOpenHelper {
  ...
  private static final String SQL_CREATE =
      "CREATE TABLE " + TABLE_HOME + " (" + COLUMN_TIME +
      " INTEGER PRIMARY KEY," + COLUMN_TYPE + " VARCHAR," + COLUMN_LABEL +
      " VARCHAR," + COLUMN_X_POS + " INTEGER," + COLUMN_Y_POS + " INTEGER," +
      COLUMN_DATA + " VARCHAR," + COLUMN_PAGE + " INTEGER," + COLUMN_DESKTOP +
      " INTEGER," + COLUMN_STATE + " INTEGER)";
  ...
  public void createItem(Item item, int page,
                         Definitions.ItemPosition itemPosition) {
    Log.i(this.getClass().getName(),
          String.format("createItem: %s (ID: %d)", item.getLabel(),
                        item.getId()));
    ...
    case APP:
    case SHORTCUT:
      Tool.saveIcon(_context, Tool.drawableToBitmap(item.getIcon()),
    ...
    case ACTION:
      ...
      break;
    case WIDGET:
      concat = item.getWidgetValue() + Definitions.DELIMITER + item.getSpanX() +
               Definitions.DELIMITER + item.getSpanY();
      itemValues.put(COLUMN_DATA, concat);
      break;
    }
  ...
  public void saveItem(Item item) { updateItem(item); }

  public void saveItem(Item item, Definitions.ItemState state) {
    updateItem(item, state);
  }

  public void saveItem(Item item, int page,
                       Definitions.ItemPosition itemPosition) {
    String SQL_QUERY_SPECIFIC =
        SQL_QUERY + TABLE_HOME + " WHERE " + COLUMN_TIME + " = " + item.getId();
  ...
  public void deleteItem(Item item, boolean deleteSubItems) {
    ...
    _db.delete(TABLE_HOME, COLUMN_TIME + " = ?",
    ...
    if (cursor.moveToFirst()) {
      ...
      do {
        ...
        if (desktopVar == ItemPosition.Desktop.ordinal() &&
            stateVar == ItemState.Visible.ordinal()) {
  ...
  public Item getItem(int id) {
    String SQL_QUERY_SPECIFIC =
        SQL_QUERY + TABLE_HOME + " WHERE " + COLUMN_TIME + " = " + id;
    Cursor cursor = _db.rawQuery(SQL_QUERY_SPECIFIC, null);
    Item item = null;
    if (cursor.moveToFirst()) {
  ...
  public void updateItem(Item item) {
    Log.i(this.getClass().getName(),
          String.format("updateItem: %s %d", item.getLabel(), item.getId()));
    ...
    _db.update(TABLE_HOME, itemValues, COLUMN_TIME + " = " + item.getId(),
               null);
  }
  ...
  public void updateItem(Item item, int page,
                         Definitions.ItemPosition itemPosition) {
    Log.i(this.getClass().getName(),
          String.format("updateItem: %s %d", item.getLabel(), item.getId()));
  ...
  private Item getSelection(Cursor cursor) {
    Item item = new Item();
    int id =
        Integer.parseInt(cursor.getString(cursor.getColumnIndex(COLUMN_TIME)));
    Item.Type type =
        Item.Type.valueOf(cursor.getString(cursor.getColumnIndex(COLUMN_TYPE)));
    String label = cursor.getString(cursor.getColumnIndex(COLUMN_LABEL));
    int x =
        Integer.parseInt(cursor.getString(cursor.getColumnIndex(COLUMN_X_POS)));
    int y =
        Integer.parseInt(cursor.getString(cursor.getColumnIndex(COLUMN_Y_POS)));
  ...
  public void open() { _db = getWritableDatabase(); }
}
