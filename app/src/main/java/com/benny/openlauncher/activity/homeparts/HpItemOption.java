...
public final void onRemoveItem(@NonNull Item item) {
	...
	if (item._location.equals(ItemPosition.Group)) {
		...
		coordinateToChildView = desktop.getCurrentPage().coordinateToChildView(
			new Point(item._x, item._y));
		desktop.removeItem(coordinateToChildView, true);
	} else {
		Dock dock = _homeActivity.getDock();
		coordinateToChildView =
			dock.coordinateToChildView(new Point(item._x, item._y));
...
public final void onInfoItem(@NonNull Item item) {
	if (item._type == Item.Type.APP) {
		try {
			...
			_homeActivity.startActivity(
				new Intent(str, Uri.parse(stringBuilder.toString())));
		} catch (Exception e) {
...
public void onRename(String name) {
	...
	if (_item._location.equals(ItemPosition.Group)) {
		...
		Desktop desktop = _homeActivity.getDesktop();
		desktop.removeItem(desktop.getCurrentPage().coordinateToChildView(point),
		                   false);
		desktop.addItemToCell(_item, _item._x, _item._y);
	} else {
		Dock dock = _homeActivity.getDock();
		_homeActivity.getDock().removeItem(dock.coordinateToChildView(point),
		                                   false);
