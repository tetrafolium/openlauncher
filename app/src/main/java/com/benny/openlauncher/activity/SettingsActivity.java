...
public class SettingsActivity extends ColorActivity
	implements SettingsBaseFragment.OnPreferenceStartFragmentCallback {
...
public void onCreate(Bundle b) {
	...
	toolbar.setNavigationIcon(
		getResources().getDrawable(R.drawable.ic_arrow_back_white));
	toolbar.setNavigationOnClickListener(v->onBackPressed());
	toolbar.setBackgroundColor(_appSettings.getPrimaryColor());

	FragmentTransaction transaction =
		getSupportFragmentManager().beginTransaction();
	transaction.replace(R.id.fragment_holder, new SettingsMasterFragment())
	.commit();
...
public boolean onPreferenceStartFragment(PreferenceFragmentCompat caller,
	...
	Fragment fragment = Fragment.instantiate(this, preference.getFragment(),
...
protected void onActivityResult(int requestCode, int resultCode,
	...
	if (resultCode == RESULT_OK) {
		...
		case Definitions.INTENT_BACKUP:
			BackupHelper.backupConfig(
				this, new File(Utils.getFileForUri(files.get(0)).getAbsolutePath() +
				               "/openlauncher.zip")
				.toString());
			Setup.dataManager().open();
			break;
		case Definitions.INTENT_RESTORE:
			BackupHelper.restoreConfig(
				this, Utils.getFileForUri(files.get(0)).toString());
			System.exit(0);
			break;
		}
	}
