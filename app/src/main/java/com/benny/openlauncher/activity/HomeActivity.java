...
public final class HomeActivity
    extends Activity implements OnDesktopEditListener {
  ...
  public static final class Companion {
    ...
    public final HomeActivity getLauncher() { return _launcher; }

    public final void setLauncher(@Nullable HomeActivity v) { _launcher = v; }
  }
  ...
  static {
    ...
    _shortcutIntentFilter.addAction(
  ...
  public final Desktop getDesktop() { return findViewById(R.id.desktop); }

  public final Dock getDock() { return findViewById(R.id.dock); }
  ...
  public final SearchBar getSearchBar() { return findViewById(R.id.searchBar); }
  ...
  public final View getStatusView() { return findViewById(R.id.status_frame); }
  ...
  private void init() {
    ...
    _appWidgetHost =
        new WidgetHost(getApplicationContext(), R.id.app_widget_host);
    ...
    hpDragOption.initDragNDrop(this, findViewById, findViewById2,
                               getItemOptionView());
  ...
  protected void initViews() {
    ...
    getDesktopIndicator().setMode(
        Setup.appSettings().getDesktopIndicatorMode());
    ...
    getDesktop().addOnPageChangeListener(new ViewPager.OnPageChangeListener() {
      ...
      public void onPageSelected(int position) {
        getDesktopOptionView().updateHomeIcon(
            appSettings.getDesktopPageCurrent() == position);
      }

      public void onPageScrollStateChanged(int state) {}
    });

    new HpAppDrawer(this, findViewById(R.id.appDrawerIndicator))
        .initAppDrawer(getAppDrawerController());
    initMinibar();
  }

  public final void initMinibar() {
    final ArrayList<LauncherAction.ActionDisplayItem> items =
        AppSettings.get().getMinibarArrangement();
  ...
  public final void initSettings() {
    ...
    // set frame colors
    getMinibarFrame().setBackgroundColor(
        appSettings.getMinibarBackgroundColor());
    ...
    getDrawerLayout().setDrawerLockMode(
        appSettings.getMinibarEnable() ? DrawerLayout.LOCK_MODE_UNLOCKED
  ...
  public final void onStartApp(@NonNull Context context, @NonNull App app,
                               @Nullable View view) {
    ...
    try {
      if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O &&
          app._userHandle != null) {
        LauncherApps launcherApps = (LauncherApps)context.getSystemService(
            Context.LAUNCHER_APPS_SERVICE);
        List<LauncherActivityInfo> activities =
            launcherApps.getActivityList(app.getPackageName(), app._userHandle);
        for (int intent = 0; intent < activities.size(); intent++) {
          if (app.getComponentName().equals(
                  activities.get(intent).getComponentName().toString()))
            launcherApps.startMainActivity(
                activities.get(intent).getComponentName(), app._userHandle,
                null, getActivityAnimationOpts(view));
        }
      } else {
  ...
  private Bundle getActivityAnimationOpts(View view) {
    ...
    if (VERSION.SDK_INT >= 23) {
      ...
      options = ActivityOptions.makeClipRevealAnimation(view, left, top, width,
                                                        height);
  ...
  public final void dimBackground() { Tool.visibleViews(200, getBackground()); }
  ...
  private void checkNotificationPermissions() {
    ...
    for (String app : appList) {
      if (app.equals(getPackageName())) {
        // Already allowed, so request a full update when returning to the home
        // screen from another app.
        Intent i = new Intent(NotificationListener.UPDATE_NOTIFICATIONS_ACTION);
        i.setPackage(getPackageName());
        i.putExtra(NotificationListener.UPDATE_NOTIFICATIONS_COMMAND,
                   NotificationListener.UPDATE_NOTIFICATIONS_UPDATE);
        sendBroadcast(i);
        return;
      }
    }
    ...
    DialogHelper.alertDialog(
        ...
        getString(R.string.notification_summary), getString(R.string.enable),
  ...
  private void handleLauncherResume() {
    if (ignoreResume) {
      // only triggers when a new activity is launched that should leave
      // launcher state alone uninstall package activity and pick widget
      // activity
      ignoreResume = false;
    } else {
      ...
      } else if (getAppDrawerController().getDrawer().getVisibility() ==
                 View.VISIBLE) {
        closeAppDrawer();
  ...
  public final void openAppDrawer() { openAppDrawer(null, 0, 0); }
  ...
  public final void closeAppDrawer() { getAppDrawerController().close(cx, cy); }
}
