...
public class HpDragOption {
  public void initDragNDrop(@NonNull final HomeActivity _homeActivity,
                            @NonNull final View leftDragHandle,
                            @NonNull final View rightDragHandle,
                            @NonNull final ItemOptionView dragNDropView) {
    ...
    dragNDropView.registerDropTarget(new DropTargetListener() {
      ...
      public void onStartDrag(Action action, PointF location) {
        ...
        if (Setup.appSettings().getDesktopShowGrid()) {
          ...
          for (CellContainer cellContainer :
               _homeActivity.getDesktop().getPages()) {
      ...
      @Override
      public void onDrop(Action action, PointF location, Item item) {
        // this statement makes sure that adding an app multiple times from the
        // app drawer works the app will get a new id every time
        ...
        if (_homeActivity.getDesktop().addItemToPoint(item, x, y)) {
          ...
          HomeActivity._db.saveItem(item,
                                    _homeActivity.getDesktop().getCurrentItem(),
                                    Definitions.ItemPosition.Desktop);
        } else {
          Point pos = new Point();
          _homeActivity.getDesktop().getCurrentPage().touchPosToCoordinate(
              pos, x, y, item._spanX, item._spanY, false);
          View itemView =
              _homeActivity.getDesktop().getCurrentPage().coordinateToChildView(
                  pos);
          if (itemView != null &&
              Desktop.handleOnDropOver(
                  _homeActivity, item, (Item)itemView.getTag(), itemView,
                  _homeActivity.getDesktop().getCurrentPage(),
                  _homeActivity.getDesktop().getCurrentItem(),
                  Definitions.ItemPosition.Desktop,
                  _homeActivity.getDesktop())) {
            _homeActivity.getDesktop().consumeLastItem();
            _homeActivity.getDock().consumeLastItem();
          } else {
      ...
      @Override
      public void onMove(Action action, PointF location) {
        _homeActivity.getDesktop().updateIconProjection((int)location.x,
                                                        (int)location.y);
      }
    ...
    dragNDropView.registerDropTarget(new DropTargetListener() {
      ...
      public void onDrop(Action action, PointF location, Item item) {
        ...
        if (_homeActivity.getDock().addItemToPoint(item, x, y)) {
          ...
          Point pos = new Point();
          _homeActivity.getDock().touchPosToCoordinate(pos, x, y, item._spanX,
                                                       item._spanY, false);
          View itemView = _homeActivity.getDock().coordinateToChildView(pos);
          if (itemView != null) {
            if (Desktop.handleOnDropOver(
                    _homeActivity, item, (Item)itemView.getTag(), itemView,
                    _homeActivity.getDock(), 0, Definitions.ItemPosition.Dock,
                    _homeActivity.getDock())) {
              _homeActivity.getDesktop().consumeLastItem();
              _homeActivity.getDock().consumeLastItem();
            } else {
              Tool.toast(_homeActivity, R.string.toast_not_enough_space);
      ...
      @Override
      public void onMove(Action action, PointF location) {
        _homeActivity.getDock().updateIconProjection((int)location.x,
                                                     (int)location.y);
      }
