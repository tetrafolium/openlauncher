...
public class AlphaSlider extends AbsCustomSlider {
  ...
  private Paint clearingStroke = PaintBuilder.newPaint()
                                     .color(0xffffffff)
  ...
  public AlphaSlider(Context context) { super(context); }
  ...
  protected void createBitmaps() {
    super.createBitmaps();
    alphaPatternPaint.setShader(
        PaintBuilder.createAlphaPatternShader(barHeight * 2));
    clearBitmap = Bitmap.createBitmap(getMeasuredWidth(), getMeasuredHeight(),
  ...
  protected void drawHandle(Canvas canvas, float x, float y) {
    ...
    if (value < 1) {
      ...
      // happens when alpha pattern is drawn underneath a circle with the same
      // size
      clearBitmapCanvas.drawColor(0, PorterDuff.Mode.CLEAR);
      clearBitmapCanvas.drawCircle(x, y, handleRadius * 0.75f + 4,
                                   alphaPatternPaint);
      clearBitmapCanvas.drawCircle(x, y, handleRadius * 0.75f + 4, solid);

      clearStroke = PaintBuilder.newPaint()
                        .color(0xffffffff)
      ...
      clearBitmapCanvas.drawCircle(
          x, y, handleRadius * 0.75f + (clearStroke.getStrokeWidth() / 2),
          clearStroke);
