...
public class ColorPickerView extends View {
  ...
  private TextWatcher colorTextChange = new TextWatcher() {
    ...
    public void onTextChanged(CharSequence s, int start, int before,
      ...
      try {
        ...
        // set the color without changing the edit text preventing stack
        // overflow
  ...
  private void initWith(Context context, AttributeSet attrs) {
    final TypedArray typedArray = context.obtainStyledAttributes(
        attrs, R.styleable.ColorPickerPreference);

    density = typedArray.getInt(R.styleable.ColorPickerPreference_density, 10);
    initialColor = typedArray.getInt(
        R.styleable.ColorPickerPreference_initialColor, 0xffffffff);

    pickerColorEditTextColor = typedArray.getInt(
        R.styleable.ColorPickerPreference_pickerColorEditTextColor, 0xffffffff);

    WHEEL_TYPE wheelType = WHEEL_TYPE.indexOf(
        typedArray.getInt(R.styleable.ColorPickerPreference_wheelType, 0));
    ColorWheelRenderer renderer =
        ColorWheelRendererBuilder.getRenderer(wheelType);

    alphaSliderViewId = typedArray.getResourceId(
        R.styleable.ColorPickerPreference_alphaSliderView, 0);
    lightnessSliderViewId = typedArray.getResourceId(
        R.styleable.ColorPickerPreference_lightnessSliderView, 0);
  ...
  protected void onLayout(boolean changed, int left, int top, int right,
    ...
    if (alphaSliderViewId != 0)
      setAlphaSlider(
          (AlphaSlider)getRootView().findViewById(alphaSliderViewId));
    if (lightnessSliderViewId != 0)
      setLightnessSlider(
          (LightnessSlider)getRootView().findViewById(lightnessSliderViewId));
  ...
  protected void onDraw(Canvas canvas) {
    ...
    float maxRadius =
        canvas.getWidth() / (1f + ColorWheelRenderer.GAP_PERCENTAGE);
    ...
    if (colorWheel != null && currentColorCircle != null) {
      ...
      // a separate canvas is used to erase an issue with the alpha pattern
      // around the edges draw circle slightly larger than it needs to be, then
      // erase edges to proper dimensions
      currentColorCanvas.drawCircle(currentColorCircle.getX(),
                                    currentColorCircle.getY(), size + 4,
                                    alphaPatternPaint);
      currentColorCanvas.drawCircle(currentColorCircle.getX(),
                                    currentColorCircle.getY(), size + 4,
                                    colorWheelFill);

      selectorStroke = PaintBuilder.newPaint()
                           .color(0xffffffff)
      ...
      if (showBorder)
        colorWheelCanvas.drawCircle(
            currentColorCircle.getX(), currentColorCircle.getY(),
            size + (selectorStroke.getStrokeWidth() / 2f), selectorStroke);
  ...
  public int getSelectedColor() {
    ...
    if (currentColorCircle != null)
      color =
          Utils.colorAtLightness(currentColorCircle.getColor(), this.lightness);
    return Utils.adjustAlpha(this.alpha, color);
  }

  public Integer[] getAllColors() { return initialColors; }
  ...
  public void setLightness(float lightness) {
    ...
    this.initialColor =
        Color.HSVToColor(Utils.alphaValueAsInt(this.alpha),
                         currentColorCircle.getHsvWithLightness(lightness));
    if (this.colorEdit != null)
      this.colorEdit.setText(
          Utils.getHexString(this.initialColor, this.alphaSlider != null));
  ...
  public void setAlphaValue(float alpha) {
    ...
    this.alpha = alpha;
    this.initialColor = Color.HSVToColor(
  ...
  public void setColorPreview(LinearLayout colorPreview,
    ...
    for (int i = 0; i < children; i++) {
      ...
      ImageView childImage =
          (ImageView)childLayout.findViewById(R.id.image_preview);
  ...
  private void setColorPreviewColor(int newColor) {
    if (colorPreview == null || initialColors == null ||
        colorSelection > initialColors.length ||
        initialColors[colorSelection] == null)
      return;
