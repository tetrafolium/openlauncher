...
public void draw() {
	...
	for (int i = 0; i < density; i++) {
		...
		float size = Math.max(1.5f + strokeWidth,
		                      cSize + (i == 0 ? 0 : cSize * sizeJitter * jitter));
		int total = Math.min(calcTotalCount(radius, size), density * 2);

		for (int j = 0; j < total; j++) {
			double angle =
				Math.PI * 2 * j / total + (Math.PI / total) * ((i + 1) % 2);
			...
			colorWheelRenderOption.targetCanvas.drawCircle(x, y, size - strokeWidth,
			                                               selectorFill);
